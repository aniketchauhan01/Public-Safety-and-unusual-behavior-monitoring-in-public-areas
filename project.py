...
# from tensorflow.python.keras.utils import to_categorical,Conv2D,MaxPooling2D, Flatten, LSTM, Dense
...
#@title Select Randomness
seed_constant = 5
np.random.seed(seed_constant)
random.seed(seed_constant)
tf.random.set_seed(seed_constant)

#@title view the dataet
# Create a Matplotlib figure and specify the size of the figure.
plt.figure(figsize = (20, 20))

DATASET_DIR = "Dataset"
# Get the names of all classes/categories in UCF50.
all_classes_names = [folder for folder in os.listdir(DATASET_DIR) if os.path.isdir(os.path.join(DATASET_DIR, folder))]
...
rows = math.ceil(n_classes / 4)
plt.figure(figsize=(20,rows*5))
for counter, random_index in enumerate(range(n_classes), 1):
...
# Ignore files like .DS_Store if they exist in video files
    video_files_names_list = [file for file in video_files_names_list if not file.startswith('.')]
    ...
    # Display the frame.
    plt.subplot(rows, 4, counter);plt.imshow(rgb_frame);plt.axis('off')

    plt.show()
#@title set dataset variables
...
CLASSES_LIST = ["WalkingwithDog", "PullUps", "Punch"]
...
#@title Plot Loss and Accuracy Graphs while Training
